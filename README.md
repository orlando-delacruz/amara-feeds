...
- **Store context default (2026-09-21, fifth batch) — complete:** the admin store context now defaults to **All stores** — a combined view with a Store column on inventory, riders, vehicles, expenses, receiving history, and the sales list (DEC-041). Write flows that need a single store (sale checkout, receiving record, payment) are disabled with a pointing hint to More → Store context until a concrete store is chosen; the Amara/Zeann options on the More page switch behave as before. Header badge hides in all-stores mode; staff stay locked to their store.
- **Next:**- **Next:**** the admin's Amara/Zeann switch was removed from every page top (credit detail, inventory, receiving, riders, vehicles, expenses) and lives once on the admin More page under "Store context" (DEC-040); all admin pages follow that context, and staff remain store-locked with no switch.
- **Owner credential (2026-09-21) — complete:** the production owner login now uses an email-shaped handle with a rotated password (DEC-042); the login resolver accepts a full email as-is and bare usernames still follow the `@zafone.local` convention (staff unaffected). Dev seed credentials (tests) keep `alice/ben/owner`, which do not touch the production account.
