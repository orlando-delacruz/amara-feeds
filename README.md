...
- **No hosted Supabase project is linked yet** — the local Supabase stack runs the migrations and proofs; linking a hosted project and setting `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` (copy `.env.example` → `.env`) switches the services to real Auth + database. Without those env vars the app falls back to the mock (test/preview path).
