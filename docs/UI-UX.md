...
- **Constraints.** The unit price is automatic per store (the stock row's current selling price, editable by admins, otherwise the latest received stock receipt); no manual price entry on the sale form. Catalog cards show the on-hand quantity for the selling store ("On hand: N", DEC-046); products never received at the store stay hidden (DEC-037). A successful save triggers the confirmed stock deduction behavior (REQ-INV-002). Saved sales are corrected by admins only (DEC-050): the sales list's admin-only Undo action voids the sale — stock returns to the store and any credit or payments recorded against it are undone (kept for the audit trail). Staff cannot edit, delete, or undo saved sales. No sale editing, cancellation, or reversal beyond the admin Undo is defined here.
...
- **Credit corrections (DEC-050).** Saved credits are corrected by admins only: the credit detail page carries an admin-only "Undo credit" action that voids the credit, its payment history, and its underlying sale (stock returns only for charge-sale credits; encoded existing-credit records never touch inventory). Voided records are kept for the audit trail, exclude from balances, history, and summaries, and surface in History ("Credit undone", "Sale corrected (undone)"). Staff cannot edit, delete, or undo saved credits.
...
- **Behavior.** Admin users consume sales by store, overall sales, outstanding credit, payments, current stock, received stock, and a "Sales by mode of payment" breakdown over a selected **From/To** date range (REQ-DASH-001 through REQ-DASH-006); the Excel export covers the same range and includes the mode of payment per sale (REQ-REP-001) plus a second "Credit" sheet—non-voided credit obligations created in the range, one row each with customer, origin store, created date, due date, original amount, amount paid, remaining balance, and status (DEC-052)—plus a third "Expenses" sheet with one row per date per store (date, store, fuel, repair, total) for recorded expenses in the range (DEC-054). Staff dashboards show today's sales, stock, and weekly sales (the Outstanding credit card is hidden from staff, DEC-058). The History destination shows an unread indicator until it is opened, and admins can filter History by store (All stores / Amara / Zeann), with business-wide events shown only under All stores. Reports are admin-only: staff never see the Reports route or navigation entry (REQ-REP-004). The admin dashboard's Current stock and Received stock lists preview at most five rows with "View all" buttons leading to Inventory and Receiving respectively (DEC-046).
