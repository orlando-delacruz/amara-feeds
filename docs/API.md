...
- **Expense operations:** record store-specific fuel/repair expenses against a rider or vehicle (at least one required); list expenses by store; retrieve per-rider and per-vehicle net summary (delivered-sales value minus expenses). Staff scope to own store; admin scope to selected store via the store control. The Expenses page additionally offers a summary-only Excel import (Date, Type, Amount, Rider, Vehicle, Note columns, validated per row) grouped by date with its own Excel export; imported rows are never recorded (client revision, DEC-054).
