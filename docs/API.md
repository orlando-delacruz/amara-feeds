...
- **Customer operations:** find or select an existing shared customer; add a new shared customer during a sale or from the customer area (customer selection is optional for sales); edit an incorrect name, contact, or address; delete an unreferenced customer (refused for customers with sales or credit records, DEC-049).
