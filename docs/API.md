...
- **Customer operations:** find or select an existing shared customer; add a new shared customer during a sale or from the customer area (customer selection is optional for sales); edit an incorrect name, contact, or address; admin-only hard delete of a customer — the customer's sales, credit obligations, and payments are removed with the customer row, refused only while an outstanding (non-voided, unpaid) credit balance exists (client revision, DEC-053; supersedes the DEC-049 refusal).
...
- **Credit operations:** raise a shared credit obligation from a charge sale under selected payment terms with an automatically calculated due date. Admins can additionally encode a customer's existing pre-system balance with complete transaction details — item lines (customer, date, item, quantity, price, total), an admin-set due date with no payment terms, and an optional initial partial payment through the normal payment flow — with no stock or receiving effect (client revision, DEC-049). Credits support partial payments, fully-paid status, and remaining balance; encoded credits never appear in sales lists, dashboards, or reports.
...
- **Inventory operations:** read store-specific stock; stock changes automatically through sale deduction and receiving, with admin manual corrections where confirmed. An approved row's quantity and current selling price are edit/delete-locked for store staff — only admins can change either — while receiving into approved rows stays allowed (client revisions, DEC-048/DEC-049).
...
- **Product operations:** staff submit products (entering pending state); admin approves products into active state; admin rejection is soft (DEC-051): the record is kept with its receiving and stock history intact and hidden from the catalog and pending approvals — rejection of a product that already carries records never fails on a foreign key.
...
- **Expense operations:** record store-specific fuel/repair expenses against a rider or vehicle (at least one required); list expenses by store; retrieve per-rider and per-vehicle net summary (delivered-sales value minus expenses); summarize recorded expenses by date; admin-only edit (target, type, amount, note; store never changes) and admin-only delete of a recorded expense (single-row removal, nothing references expenses). Staff scope to own store; admin scope to selected store via the store control. The report Excel export carries a per-date-per-store Expenses sheet (client revision, DEC-054).
...
| 8 | Exact report/export behavior (columns, formats) | **Assumed implemented** | Summaries confirmed; per-sale sales sheet implemented (REQ-REP-001) plus a Credit sheet for credit obligations created in the range (DEC-052) and an Expenses sheet with one row per date per store (DEC-054); further column/format changes Confirmation Required. |
