...
- **Credit operations:** raise a shared credit obligation from a charge sale under selected payment terms with an automatically calculated due date. Admins can additionally encode a customer's existing pre-system balance as a balance-only obligation — no originating sale, no payment terms (an admin-set due date and origin store instead), and no stock or receiving effect; it is settled through the normal payment flow (client change, DEC-048).
...
- **Inventory operations:** read store-specific stock; stock changes automatically through sale deduction and receiving, with admin manual corrections where confirmed. Admin-approved stock rows are edit/delete-locked for store staff — only admins can change them — while receiving into approved rows stays allowed (client change, DEC-048).
