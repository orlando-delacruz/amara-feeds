...
### DEC-050 — Bank-style admin-only correction: void/undo for sales, credits, and inventory (migration 00010)

- **ID:** DEC-050
- **Title:** Bank-style admin-only correction: void/undo for sales, credits, and inventory (migration 00010)
- **Status:** Accepted
- **Date:** 2026-09-21
- **Context:** Client clarified the correction model: staff encode and save transactions, but only the admin corrects, undoes, deletes, or adjusts them — like a bank workflow. DEC-049's sale deletion was a hard delete that refused sales whose credit had payments, so a paid sale could never be corrected, and inventory correction still allowed staff edits on unapproved rows — both contradicting the client's permission table (Correct/Delete Inventory: staff ❌; Edit/Delete/Undo Sale and Credit: staff ❌, admin ✅).
- **Decision:** Migration `00010`, verified by the extended Gate-4 proof suite (26/26). Corrections become reversible reversals instead of deletions: (1) `sales.is_voided` and `payments.is_voided` (default false — additive, live rows unaffected) and credit status gains `'voided'`. (2) `void_sale` (admin-only) replaces `delete_sale`: the sale is voided — kept for the audit trail, excluded from every list/summary — its deducted stock is restored exactly once, and the linked credit plus its payment rows are voided (payments remain as history, excluded from balances, history, and summaries); re-void is refused, so stock can never be double-restored. (3) `void_credit` (admin-only) undoes a credit from the credit side the same way; only charge-sale credits restore stock — encoded legacy credits never touched inventory, so undoing them has none. (4) `adjust_stock`/`delete_stock` become admin-only outright (superseding the DEC-032 staff carve-out for unapproved stock); staff keep receiving through the normal workflow and keep the ability to create sales and record payments. (5) `record_payment` refuses voided credits. UI: the admin's sale action is "Undo", the credit detail page gains an admin-only "Undo credit", staff inventory rows show "Admin-managed" with no actions; corrections surface in History ("Sale corrected (undone)", "Credit undone"). Supersedes DEC-049's sale-deletion refusals and DEC-032; amends DEC-048's approval-lock rationale.
- **Alternatives considered:** Hard deletion of paid sales with their payments — rejected: destroys financial history. In-place sale editing — rejected: an editor that mutates saved lines is harder to keep consistent than void-and-re-encode, and every listed mistake case is covered by undo + re-encode. Keeping staff edits on unapproved stock — rejected: the client's permission table is explicit.
...
### DEC-054 — Expenses by-date summary on the page + Expenses sheet in the report export (no schema change)

- **ID:** DEC-054
- **Title:** Expenses by-date summary on the page + Expenses sheet in the report export (no schema change)
- **Status:** Accepted
- **Date:** 2026-09-22
- **Context:** Final client revision for Expenses: summarize recorded expenses by date and export the summary as Excel. The client first asked for an Excel *import*, then corrected the direction to *export* (recorded data out, not file data in) — the briefly-built import approach (Excel upload parsed in memory) was reverted before release. Verified first: the expenses schema carries no expense-date field (only server-set `created_at`), so each record counts on its recording date.
- **Decision:** No database changes, no new dependencies (the `read-excel-file` spike was uninstalled). (1) The Expenses page gains a "Summary by date" section grouping the store-scoped recorded expenses (`src/features/expenses/expenseSummary.ts`, pure grouping over `createdAt` dates) into Date/Fuel/Repair/Total rows. (2) The admin report export gains a third "Expenses" sheet: `buildExpenseReportRows` groups both stores' recorded expenses per date per store within the report's From–To range (same Option-A range semantics as the Credit sheet, DEC-052), written by the existing `exportReportExcel` writer; the export toast reads "Sales + Credit + Expenses".
